...
В файле idf_component.yml указываются только те компоненты, которые вы добавили вручную (прямые зависимости). Компонент esp_bsp_generic сам по себе является «сборником». В его собственном файле манифеста уже прописаны lvgl, esp_lcd_gc9a01 и другие драйверы. Менеджер компонентов видит это и скачивает их автоматически как транзитивные (вложенные) зависимости. Вам не нужно засорять свой idf_component.yml десятком строк. Достаточно одной «родительской» зависимости, которая потянет за собой всё дерево нужных библиотек.

Полный список зависимостей можно посмотреть в файле dependencies.lock.
...
Она автоматически использует настройки из menuconfig для инициализации SPI-шины, контроллера дисплея и графической библиотеки LVGL.
